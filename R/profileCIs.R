profileCIs <- function(x, y, wt, B, B.inf, side, alpha, l.max, step, useAstar, penalized, tol = 1e-03, verbose=FALSE, ...) {
   ...
   for (i in 1:(p * J)) {
      ...
      while (l >= l.null & iter <= 300) {
         ...
         temp <- profile(x, y, wt, B, i, tol, useAstar, penalized); B <- temp$B; l <- temp$l
         }
         vecB <- vec(t(B))
 if (verbose) { cat(t(vecB), l, iter, "profile CIsA \n") } ;
 ...
 if (verbose) { cat(t(vecB), l, iter, "profile CIsB \n") } ;
         iter <- iter + 1
      }
      ...
      temp <- SASalgo(x, y, wt, B, l.null, tol, i, side, useAstar, penalized, verbose=verbose); B <- temp$B ; converge <- temp$converge
